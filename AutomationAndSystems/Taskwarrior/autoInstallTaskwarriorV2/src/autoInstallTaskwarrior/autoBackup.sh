...
backupPath="/mnt/e/18-09-19 Document structure/personal/Automation and Systems/taskwarrior/twBackups/"
...
if [ -d "$backupPath"$(date +%Y%m%d%H)$(($(date +%M)-1))"/scheduledCronjobs/" ]; then
	sudo cp -r "$userFolder"/maintenance/autoBackup.sh "$backupPath"$(date +%Y%m%d%H)$(($(date +%M)-1))"/maintenance/"
	echo "Backup 5/5 successfull for previous minute"
fi
if [ -d "$backupPath"$(date +%Y%m%d%H%M)"/scheduledCronjobs/" ]; then
	sudo cp -r "$userFolder"/maintenance/autoBackup.sh "$backupPath"$(date +%Y%m%d%H%M)"/maintenance/"

...
/**
 * A list of the verified commands (In the comment above each command) is
 * converted into a set of commands that Java can execute (an array of strings).
 * 
 * At the second last entry of a command, the accompanying "working path" is
 * specified in method generateCommands. At the last entry the environment
...
//		commandLines[0][5] = "TASKDDATA=/var/taskd";
		...
		// Note: the /var folder should be located in root. Hence, it should be/is an
		// absolute path.
		// working folder should be root
		// environment variable:TASKDDATA=/var/taskd
		// TODO: verify that there is no /var/taskd made in the current working
		// directory/location of the .jar file.
...
//		commandLines[7][5] = "TASKDDATA=/var/taskd";
		...
		// working folder should be root
		// environment variable:TASKDDATA=/var/taskd
		// Verified by checking whether /var/taskd/config did not exist before execution
		// of
		// this command, and does exist after. Status: Both verified, hence command
		// verified.
		...
		/**
		 * dir contains: /usr/share/taskd/pki$ dir generate generate.ca generate.client
		 * generate.crl generate.server vars
		 *
		 * After command ./generate: usr/share/taskd/pki$ dir generate generate.ca
		 * generate.client generate.crl generate.server vars
		 */

		/**
		 * substitute sudo ./generate to: sudo /usr/share/taskd/pki//generate The
		 * ./generate command means execute "generate" in the current path source:
		 * https://unix.stackexchange.com/questions/249039/what-means-the-dots-on-a-path
		 * Source1:
		 * https://askubuntu.com/questions/74780/how-to-execute-script-in-different-directory
		 * The dot does not mean execute, it means current directory Since the current
		 * directory is where this .jar file is located, it does not contain a
		 * /usr/share.. path (that only exists in root{unless you specifically made it
		 * somewhere else}).
		 */
		// pdf:9a
		// sudo ./generate
		...
		/**
		 * TODO: derive where the client.cert.pem is actually generated/located Or where
		 * the installation directory currently would be and copy the client.cert.pem
		 * from that absolute path with for example: sudo cp
		 * /user/someactualdirectory/client.cert.pem "/var/task/"
		 *
		 */
		// pdf:9b
...
//		sudo taskd add user 'Public' 'First' --data $TASKDDATA
		...
		/**
		 * Generate commands of 18 (pdf) on. Add working directory at the second last
		 * end of the command array. Last entry contains the environment variable.
		 * 
		 * @param testRun
		 * @param linuxPath
		 * @param vars
		 ...
		 * @return
		 */
		// public static String[][] generateSecondCommands(InstallData installData,
		// boolean testRun,String linuxPath,String vars,String[] storeUserInput,String
		// serverName,String serverPort) {
...
//			commands[33].setWorkingPath("");
			...
			// to: sudo task config taskd.certificate -- /home/<Ubuntu
			// username>/.task/<taskwarrior username>.cert.pem
			// working directory: /usr/share/taskd/pki/
			...
			commandLines[38][4] = "--";
			commandLines[38][5] = "/home/" + installData.getLinuxUserName() + "/.task/" + installData.getTwUserName()
					+ ".cert.pem";
			...
			// to: sudo task config taskd.key -- /home/<Ubuntu username>/.task/<taskwarrior
			// username>.key.pem
			// working directory: /usr/share/taskd/pki/
			...
			commandLines[39][4] = "--";
			commandLines[39][5] = "/home/" + installData.getLinuxUserName() + "/.task/" + installData.getTwUserName()
					+ ".key.pem";
			...
			// pdf:21a
			// sudo task config taskd.credentials --
			// Public/First/b6634b72-04a3-4220-8d49-6d56as4tefb
			// to: sudo task config taskd.credentials -- <taskwarrior
			// organisation>/<taskwarrior username>/<taskwarrior uuid>
			// working directory: /usr/share/taskd/pki/
			...
			commandLines[42][4] = "--";
			commandLines[42][5] = installData.getTwOrganisation() + "/" + installData.getTwUserName() + "/"
					+ installData.getTwUuid();
...
//	
		/**
		 * The second last entry contains the working path the last entry contains the
		 * environment variable Set the environment path with the command.
		 * 
		 * @param testRun
		 * @param linuxPath
		 * @param vars
		 ...
		 * @return
		 */
		// public static String[][] generateThirdCommands(InstallData installData,
		// boolean testRun,String linuxPath,String vars,String[] storeUserInput,String
		// serverName,String serverPort) {
		// String[][] commands = new String[50][1];
		String twUuid;
		// String directoryPath = new
		// String("/var/taskd/orgs/"+installData.getTwOrganisation()+"/users/");
		// String directoryPath = new
		// String("/var/taskd/orgs/"+installData.getTwUuid()+"/users/");
		...
		if (!installData.isTestrun()) {
			...
			// TODO: Add to enable automatic sync: sudo task config taskd.trust -- ignore
			// hostname
			...
			commandLines[58][2] = "/home/" + installData.getLinuxUserName() + "/maintenance/"
					+ installData.getVisudoFileName();
...
//			
			/**
			 * Copy/create the custom sort script. pdf: no, see git Merge project customsort
			 * in here.
			 */

			/**
			 * Make a cronjob that handles automatic backup and customSort script pdf: 10
			 * sec: 6.2 ignored, (see git readme "login and start cronjob service
			 * automatically")
			 * 
...
//				*/10 * * * * root sh -v /home/a/maintenance/customSort.sh
			...
			/**
			 * Enable crontab services at startup with bashrc. pdf: 10 sec: 6.3
			 * 
			 * 1. cd to `/home/<your ubuntu user name>` 2. `sudo nano .bashrc` 3. The text
			 * editor nano then creates/opens a file `.bashrc` 4. In that file a lot of
			 * examples can be shown already, write/paste the following code on the first
			 * line/above what was already there in the `.bashrc` file.
			 * 
			 * 2 options: server or non-server/client
			 */

			/**
			 * remove prompting for password with sudo visudo. That changes
			 * /etc/sudoers.tmp. TODO: Add last line to that file with: <your WSL ubuntu
